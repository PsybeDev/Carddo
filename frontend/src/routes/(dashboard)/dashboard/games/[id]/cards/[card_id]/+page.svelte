<script lang="ts">
	...
	import type { Card, Game, GameConfig, PropertyConfig, ZoneConfig } from '$lib/types/api';
	...
	function normalizeConfig(raw: Game['config'] | undefined): GameConfig {
	...
	function normalizeZones(value: unknown): ZoneConfig[] {
	...
	function normalizeProperties(value: unknown): PropertyConfig[] {
		if (!Array.isArray(value)) return [];
		return value.map((p) => {
			...
			return { name: typeof obj.name === 'string' ? obj.name : '', default: defaultValue };
		});
	}
