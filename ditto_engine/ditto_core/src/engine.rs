...
impl GameState {
    ...
    /// Like [`resolve_queue`], but returns an error if more than `max_steps` events
    /// are resolved before the queue drains, guarding against runaway hook chains.
    pub fn resolve_queue_bounded(&mut self, max_steps: usize) -> Result<(), String> {
        ...
        while let Some(event) = match order {
            ...
            if steps >= max_steps {
                return Err(format!("resolution limit of {max_steps} steps exceeded"));
            }
            steps += 1;
